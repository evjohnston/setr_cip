...
from setr import setr_cip_map  # External CIP-to-SETR mapping
...
# Output directory
os.makedirs("SETRtables", exist_ok=True)

# Collect combined data for final summary
...
# Process each year and degree level
for YEAR in YEARS:
    ...
    resident_cols = [col for col in df.columns if "total" in col.lower()
                     and "nonresident" not in col.lower()
    ...
    for level_key, level_value in DEGREE_LEVELS.items():
        ...
        with open(f"SETRtables/top_institutions_by_category_{YEAR}_{level_key}.html", "w", encoding="utf-8") as f:
...
for level_key, dfs in combined_all.items():
    ...
    with open(f"SETRtables/top_institutions_combined_2014_2023_{level_key}.html", "w", encoding="utf-8") as f:
        f.write(str(soup.prettify()))
